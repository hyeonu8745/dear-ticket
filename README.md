...
**🌐 배포 주소: [https://jihyeonu.com](https://jihyeonu.com)**
...
AI          FastAPI · PyTorch · scikit-learn · Ollama (Qwen3 8B)
...
[User / Client]
      │
      ▼
[Cloudflare Tunnel]  ── 보안 터널 (포트포워딩 없음)
      │
      ▼
[Nginx :80]  ── 리버스 프록시 + 로드밸런싱
      │                        │
      ▼                        ▼
[React SPA]          [Spring Boot :8080]
(정적 파일 서빙)      [Spring Boot :8081]
                             │
               ┌─────────────┼─────────────┐
               ▼             ▼             ▼
         [MySQL :3306]  [Redis :6379]  [AI Servers]
                                           │
                          ┌────────────────┼────────────────┐
                          ▼                ▼                ▼                ▼
                    [:8000 봇탐지]  [:8001 추천]  [:8002 수요예측]  [:8003 챗봇]
...
|------|------|
| 서버 | 온프레미스 (로컬 PC) |
| 외부 연결 | Cloudflare Tunnel (포트포워딩 없음) |
| 프론트 서빙 | Nginx 정적 파일 서빙 (`/var/www/ticketing`) |
| 백엔드 | Spring Boot jar 직접 실행 (8080 / 8081 이중화) |
| 도메인 라우팅 | `jihyeonu.com` → Nginx :80 |
| API 라우팅 | `api.jihyeonu.com` → Nginx :80 → 8080/8081 |
| 모니터링 | `monitor.jihyeonu.com` → Grafana :3000 |
